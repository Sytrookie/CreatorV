...
local function neon()
    ...
    for i = 1, 4 do
        ...
        options[i] = {
            label = ('Neon %s'):format(VMODSConfig.Neon[i].label),
            ...
            set = function(index)
                ...
                return originalNeon[i] == (index == 2), ("Neon %s %s"):format(VMODSConfig.Neon[i].label, index == 2 and 'enabled' or 'disabled')
            end,
            restore = function()
    ...
    local rgbValues = {}
    local neonLabels = {}
    for i, v in ipairs(VMODSConfig.NeonColors) do
    ...
    options[5] = {
        ...
        rgbValues = rgbValues,
        set = function(index)
            local rgb = VMODSConfig.NeonColors[index]
            SetVehicleNeonLightsColour(vehicle, rgb.r, rgb.g, rgb.b)
            return originalLabelIndex == index, ('%s neon installed'):format(VMODSConfig.NeonColors[index].label)
        end,
        restore = function()
            local rgb = VMODSConfig.NeonColors[originalLabelIndex]
            SetVehicleNeonLightsColour(vehicle, rgb.r, rgb.g, rgb.b)
        end,
